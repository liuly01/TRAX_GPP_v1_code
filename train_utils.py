...
def run_training(
    ...
    If yoy_loader is not None and lambda_yoy > 0, an extra YoY loss term
    is added for trend component (IADC).
    """
...
def create_model_from_params(
    ...
    """
    Helper to instantiate LSTMMain from hyperparameters.
    """
...
def cv_objective_for_component(
    ...
    """
    Cross-site, cross-fold CV objective for one component.
    Returns mean validation loss across valid folds.
    """
    K_global = max(
...
def optimize_component_hyperparams(
    ...
    """
    Hyperparameter optimization wrapper for one component.
    Uses Hyperopt with CV objective.
    """
    trials = Trials()
