# -*- coding: utf-8 -*-
"""
Trend-enhanced RSTL-LSTM training with:
- Adaptive Dev + Final Test split
- Expanding-window cross-validation
- IADC (MSE + YoY) for trend component
- Strict time ordering to avoid information leakage
"""
...
RESULT_PATHS = {
    "combined_noOLS": os.path.join(BASE_DIR, "ALL_SITES_combined_test.csv"),
}
...
def read_csv(path):
    """
    Read all site CSV files under a folder.
    Each CSV must contain at least: 'time', TARGET_COLUMN, FEATURE_COLUMNS.
    Returns:
      data0: list of raw numpy arrays [N, 2 + F] (time, target, features)
      data:  list of numpy arrays [N, F+1] (features + target)
      site_names: list of site identifiers inferred from file names
    """
    data0 = []
    data = []
    site_names = []
...
def compute_rstl_for_all_sites(data):
    """
    Apply robust STL decomposition (period=12) to each column per site.
    Input:
      data: list of arrays [N, F+1] (features + target)
    Return:
      trend_data_sites, season_data_sites, resid_data_sites
      (each list element has same shape as input site array)
    """
    trend_data_sites = []
    season_data_sites = []
    resid_data_sites = []
...
def scale_all_sites(trend_data_sites, season_data_sites, resid_data_sites):
    """
    Global MinMax scaling across all sites for:
    - features and target of trend / season / resid
    Returns:
      scaled_trend_sites, scaled_season_sites, scaled_resid_sites
      and 6 scalers for features/targets.
    """
...
def compute_integrated_gradients_importance(model, X, feature_names, out_csv, baseline=None, steps=50):
    """
    Integrated Gradients feature importance on Final Test windows.
    X: tensor [N, num_features, seq_len]
    Summarize attributions across time and samples, normalize to percentage.
    """
