...
def decide_dev_test_and_folds(L, window_len):
    """
    Decide dev length, final test length and CV fold settings for one site.
    L: full time-series length (months)
    window_len: input window length (months)
    """
...
def get_rolling_window_multistep(out_len,
                                 ...
                                 labels_per_site):
    """
    Build rolling windows for each site separately.
    feats_per_site[i]: array [num_features_plus_target, T]
    labels_per_site[i]: array [1, T]
    Return:
      feature_list: list of arrays [n_win_site, window_len, num_features]
      label_list:   list of arrays [n_win_site, 1, 1]
    """
    feature_list = []
    label_list = []
...
def combine_sites_windows(feature_list, label_list):
    """
    Concatenate site-level window arrays along batch dimension.
    Input:
      feature_list: list of [n_win_site, window_len, features]
      label_list:   list of [n_win_site, 1, 1]
    Output:
      xy_combined: [N_total, window_len, features]
      y_combined:  [N_total, 1, 1]
    """
...
def build_fold_datasets_for_component(
    ...
    """
    Build cross-site Train/Val TensorDataset for one fold.
    data_transposed_list[i]: [num_features_plus_target, T]
    labels_transposed_list[i]: [1, T]
    Only dev segment is used for CV, final test is excluded.
    """
...
def build_dev_train_val_for_component(
    ...
    """
    Build final Train/Val TensorDataset on dev segment for one component.
    Train:   dev[0 : dev_len - val_len]
    Val:     dev[dev_len - val_len : dev_len]
    Final test segment is not used here.
    """
    train_feats_per_site = []
    train_labels_per_site = []
    val_feats_per_site = []
...
def _concat_yoy_pairs_single_site(xy_arr, y_arr, lag):
    """
    Build YoY pairs within one site window array (no cross-site mixing).
    xy_arr: [N, window_len, features]
    y_arr:  [N, 1, 1] or [N, 1]
    """
...
def build_yoy_datasets_for_trend(
    ...
    """
    Build YoY pair datasets (t, t-lag) for trend component.
    Inputs are per-site window lists, no cross-site mixing.
    Return:
      yoy_train_dataset: TensorDataset(X_t, X_t12, y_t, y_t12)
      yoy_val_dataset:   TensorDataset(X_t, X_t12, y_t, y_t12)
    """
    X_t_all = []
    X_t12_all = []
    y_t_all = []
